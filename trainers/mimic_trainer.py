...
from datasets.mimic_dataset import num_groups_per_attrb
...
class MIMICTrainer(LightningModule):
    def __init__(self, model, learning_rate=0.001, end_lr_factor=1.0, weight_decay=0.0, decay_steps=1000):
        ...
        self.loss = nn.BCEWithLogitsLoss()  # For binary classification, adjust if necessary
    ...
    def configure_optimizers(self):
        optimizer = torch_optimizer.Lamb(self.model.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=5, verbose=True, factor=0.5)

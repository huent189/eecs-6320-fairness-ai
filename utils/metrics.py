...
def compute_optimal_threshold(preds, true_labels):
    ...
    delta_th = 0.01
    f1_scores = [
        torchmetrics.functional.classification.binary_f1_score(
            preds, true_labels, threshold=th, validate_args=True
        )
...
class GroupBasedStats(nn.Module):
    ...
    def get_acc_per_group_var_thrs(
        ...
        for group in range(num_groups_per_attrb[attr_idx]):
            ...
            group_preds = torch.gt(preds[idx, 0], self.thres).int()
    ...
    def get_fpr_per_group_var_thrs(
        ...
        for group in range(num_groups_per_attrb[attr_idx]):
            ...
            group_true = true_labels[idx, 0]
            fpr = (
                torch.ne(group_preds, group_true).int() * group_preds
            ).sum().item() / (group_true != 1).int().sum().item()
            fprs.append(fpr)
        return fprs

    def get_tpr_per_group_var_thrs(
        self, preds, true_labels, sensitive_attrs, thresholds, attr_idx
    ):
        tprs = []
        for group in range(num_groups_per_attrb[attr_idx]):
            ...
            group_true = true_labels[idx, 0]
            tpr = (
                torch.eq(group_preds, group_true).int() * group_true
            ).sum().item() / (group_true == 1).int().sum().item()
            tprs.append(tpr)
        return tprs

    def get_fdr_per_group_var_thrs(
        self, preds, true_labels, sensitive_attrs, thresholds, attr_idx
    ):
        fdrs = []
        for group in range(num_groups_per_attrb[attr_idx]):
            ...
            group_preds = torch.gt(preds[idx, 0], self.thres).int()
            group_true = true_labels[idx, 0]
            fp = (torch.ne(group_preds, group_true).int() * group_preds).sum().item()
            tp = (torch.eq(group_preds, group_true).int() * group_true).sum().item()
            fdr = fp / (fp + tp) 
            fdrs.append(fdr)
        return fdrs
    ...
    def get_all_stats(self):
        ...
        for attr in range(group_attrbs.shape[-1]):
            ...
            tprs = self.get_tpr_per_group_var_thrs(
                y_preds, y_trues, group_attrbs, self.thres, attr
            )
            tpr_all_groups.append(tprs)
            tpr_gaps.append(max(tprs) - min(tprs))
            fdrs = self.get_fdr_per_group_var_thrs(
                y_preds, y_trues, group_attrbs, self.thres, attr
            )
            fdr_all_groups.append(fdrs)
            fdr_gaps.append(max(fdrs) - min(fdrs))
            
        metrics = {
            'acc': [accuracies_all_groups, accuracy_gaps],
            'fpr': [fpr_all_groups, fpr_gaps],
            'tpr': [tpr_all_groups, tpr_gaps],
            'fdr': [fdr_all_groups, fdr_gaps],
        }
        return metrics        
...
class GroupBasedStatsVaryingThrs(nn.Module):
    ...
    def set_thrshold_per_sample(self):
        ...
        self.threhold_per_sample = torch.tensor(
            self.threhold_per_sample, device="cuda:0", dtype=torch.float32
        )
...
class OptimalThresholdPerGroup(nn.Module):
    ...
    def get_optimal_threshold(self, y_preds, y_trues):
        ...
        ), "assertion error, no best threshold was found, change the initial value of the best loss."
        print(best_fpr)
